...
@app_commands.describe(x="Value for x (adds 29)", y="Value for y (subtracts 32)", z="Value for z (adds 50)")
async def calc(interaction: discord.Interaction, x: float, y: float, z: float):
    ...
    embed.add_field(name=f"z ({z} + 50)", value=str(z_out), inline=False)
